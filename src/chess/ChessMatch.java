...
public class ChessMatch {
	...
	public ChessPiece replacePromotedPiece(String type) {
		...
		if (!type.equalsIgnoreCase("q") && !type.equalsIgnoreCase("N") && !type.equalsIgnoreCase("r") && !type.equalsIgnoreCase("b")) {
	...
	private Piece makeMove(Position source, Position target) {
		...
		if (p instanceof King && target.getColumn() == source.getColumn() + 2) {
			...
			if (!testCanCastling(p.getColor(), p.getChessPosition().toPosition(), kingside)) {
				undoMove(source, target, capturedPiece);
	...
	private boolean testCanCastling(Color color, Position position, boolean kingside) {
		List<Piece> opponentPieces = pieceOnTheBoard.stream()
				.filter(x -> ((ChessPiece) x).getColor() == opponent(color)).collect(Collectors.toList());

		for (Piece p : opponentPieces) {
			boolean[][] mat = p.possibleMoves();
			if (mat[position.getRow()][position.getColumn() + 1] == true && kingside) {
				return false;
			} else if ((mat[position.getRow()][position.getColumn() - 1] == true
					|| mat[position.getRow()][position.getColumn() - 2] == true) && !kingside) {
				return false;
			}
		}
